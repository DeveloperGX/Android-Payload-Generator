...
install_tool() {
    ...
    if [[ "$SYSTEM" == "termux" ]]; then
        ...
        sudo echo "cd ~/.GX-Tools/Android-Payload-Generator ; ./android-p-g.sh " > $PREFIX/usr/local/bin/apg
        sleep 0.5s
        sudo chmod +x $PREFIX/usr/local/bin/apg
        sleep 0.5s
        sudo echo "cd ~/.GX-Tools/Android-Payload-Generator ; ./android-p-g.sh " > $PREFIX/usr/local/bin/APG
        sleep 0.5s
        sudo chmod +x $PREFIX/usr/local/bin/APG
        sleep 0.5s
        sudo echo "cd ~/.GX-Tools/Android-Payload-Generator ; ./android-p-g.sh " > $PREFIX/usr/local/bin/android-p-g
        sleep 0.5s
        sudo chmod +x $PREFIX/usr/local/bin/android-p-g
        sudo echo "cd ~/.GX-Tools/Android-Payload-Generator ; ./uninstaller.sh " > $PREFIX/bin/apg-uninstall
        sleep 0.5s
        sudo chmod +x $PREFIX/bin/apg-uninstall
    fi
...
main_menu() {
    ...
    echo ''
    sleep 1s
    echo -e $LGreen "The Tool Was Installation Successfull You Can Start It By "APG" or "apg" or "android-p-g" Commands (^_^) " 
    exit
}
main_menu
