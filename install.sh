...
check_root
sleep 1s;
cd ~/.GX-Tools/Android-Payload-Generator ; sudo ./android-p-g.sh " > /usr/bin/apg
        sleep 0.5s
...
check_root
sleep 1s;
cd ~/.GX-Tools/Android-Payload-Generator ; sudo ./android-p-g.sh " > /usr/bin/android-p-g
        sleep 0.5s
        sudo echo "check_root() {
    if [[ "$EUID" -ne 0 ]]; then
        echo -e "${RED}[-] Please Run This It As Root By sudo .${NC}"
...
check_root
sleep 1s;
cd ~/.GX-Tools/Android-Payload-Generator ; sudo ./uninstaller.sh " > /usr/bin/apg-uninstall
        sleep 0.5s
